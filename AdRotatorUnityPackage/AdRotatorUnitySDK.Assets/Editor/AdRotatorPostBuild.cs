    ...
    public static class AdRotatorPostBuild
    {
        ...
        public static void OnPostProcessBuild(BuildTarget target, string path)
        {
            if ((target == BuildTarget.MetroPlayer && UnityEditor.EditorUserBuildSettings.metroBuildType == MetroBuildType.VisualStudioCSharp) || (target == BuildTarget.WP8Player))
            {
                ...
                //Check if this is a Universal project, if so make it a double
                if (!File.Exists(Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".csproj")))
                {
                    // Copy XAML, XML and Targets file to the Windows 8.1 Universal folder
                    CopyFilesToProject(target, Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".Windows"), adRotatorAssetPath);
                    // Copy XAML, XML and Targets file to the Windows Phone 8.1 Universal folder
                    CopyFilesToProject(target, Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".WindowsPhone"), adRotatorAssetPath);

                    // Add Import statements and build targets in to Windows 8.1 csproj
                    UpdateCsProj(Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".Windows"), generatedCodeDir.Name + ".Windows", target, rotatorTargetFile, adSettingsTargetFile, houseAdsTargetFile, adRotatorAssetPath);
                    // Add Import statements and build targets in to Windows Phone 8.1 csproj
                    UpdateCsProj(Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".WindowsPhone"), generatedCodeDir.Name + ".WindowsPhone", target, rotatorTargetFile, adSettingsTargetFile, houseAdsTargetFile, adRotatorAssetPath);

                    // Add custom code next to already generated code in App.xaml.cs for the Shared project
                    UpdateSourceCodeWin8(Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".Shared"));

                    UpdateCapabilitiesWP8_1(Path.Combine(generatedCodeDir.FullName, generatedCodeDir.Name + ".WindowsPhone"));

                }
                else
                {
                    // Copy XAML, XML and Targets file to the proejct folder
                    CopyFilesToProject(target, generatedCodeDir.FullName, adRotatorAssetPath);

                    // Add Import statements and build targets in csproj
                    UpdateCsProj(generatedCodeDir.FullName, generatedCodeDir.Name, target, rotatorTargetFile, adSettingsTargetFile, houseAdsTargetFile, adRotatorAssetPath);

                    if (target == BuildTarget.MetroPlayer)
                    {
                        // Add custom code next to already generated code in App.xaml.cs
                        UpdateSourceCodeWin8(generatedCodeDir.FullName);
#if UNITY_WP_8_1
                        UpdateCapabilitiesWP8_1(generatedCodeDir.FullName);
#endif
                    }
                    if (target == BuildTarget.WP8Player)
                    {
                        ...
                        UpdateCapabilitiesWP8(generatedCodeDir.FullName);
                    }
                }
        ...
        private static void CopyFilesToProject(BuildTarget target, string generatedCodeDir, string adRotatorAssetPath)
        {
            if (target == BuildTarget.MetroPlayer)
            {
                File.Copy(Path.Combine(adRotatorAssetPath, "AdRotatorBridgeWin8.cs.ignore"), Path.Combine(generatedCodeDir, "AdRotatorBridge.cs"), true);
            }
            if (target == BuildTarget.WP8Player)
            {
                File.Copy(Path.Combine(adRotatorAssetPath, "AdRotatorBridgeWP8.cs.ignore"), Path.Combine(generatedCodeDir, "AdRotatorBridge.cs"), true);
            ...
            CopyFiles(adRotatorAssetPath, generatedCodeDir, "*.xml");
            CopyFiles(adRotatorAssetPath, generatedCodeDir, "*.xaml");
            CopyFiles(adRotatorAssetPath, generatedCodeDir, "*.targets");
        }
        ...
        private static void UpdateCsProj(string generatedCodeDir, string projectName, BuildTarget target, string rotatorTargetFile, string adSettingsTargetFile, string houseAdsTargetFile, string adRotatorAssetPath)
        {
            var unityProject = Path.Combine(generatedCodeDir, projectName + ".csproj");
            ...
            UpdateTargetFileContent(Path.Combine(generatedCodeDir, adSettingsTargetFile), "defaultAdSettings.xml");


            var xamlFiles = GetFiles(adRotatorAssetPath, "*.xaml");
            UpdateTargetFileXamlPage(Path.Combine(generatedCodeDir, houseAdsTargetFile), xamlFiles.Select(f => f.Name));
        ...
        private static void UpdateSourceCodeWin8(string generatedCodeDir)
        {
            var appCsFile = Path.Combine(generatedCodeDir, "App.xaml.cs");
        ...
        /// </summary>
        /// <param name="generatedCodeDir">Path of the generated source code.</param>
        private static void UpdateCapabilitiesWP8(string generatedCodeDir)
        {
            var appCapsFile = Path.Combine(generatedCodeDir, "Properties\\WMAppManifest.xml");
        ...
        /// <summary>
        /// Fix the WP 8.1 capabilities manifest as it is currently broken.
        /// </summary>
        /// <param name="generatedCodeDir">Path of the generated source code.</param>
        private static void UpdateCapabilitiesWP8_1(string generatedCodeDir)
        {
            var appCapsFile = Path.Combine(generatedCodeDir, "Package.appxmanifest");
            var appCapsFileAllLines = File.ReadAllLines(appCapsFile);
            var saveFile = false;
            for (int i = 0; i < appCapsFileAllLines.Length; i++)
            {
                var line = appCapsFileAllLines[i];
                if (line.Contains("      <Capability Name=\"internetClientServer\" />"))
                {
                    // internetClientServer capability already in project					
                    break;
                }
                if (line.Contains("<Capability Name=\"internetClient\" />"))
                {
                    appCapsFileAllLines[i] = "      <Capability Name=\"internetClientServer\" />";
                    saveFile = true;
                }
                if (line.Contains("</Capabilities>") && !saveFile)
                {
                    appCapsFileAllLines[i] = "      <Capability Name=\"internetClientServer\" />" + Environment.NewLine + line;
                    saveFile = true;
                }
                if (line.Contains("</Package>") && !saveFile)
                {
                    appCapsFileAllLines[i] = "<Capabilities>" + Environment.NewLine + "      <Capability Name=\"internetClientServer\" />" + Environment.NewLine + "</Capabilities>" + Environment.NewLine + line;
                    saveFile = true;
                }
            }
